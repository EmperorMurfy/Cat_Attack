...
//game bugs I have to fix:  fireball should move even when blocked, graphics for when they are injured without error, health reduction for fire method is way too much gotta fix that
...
public class GraphicsPanel extends JPanel implements KeyListener, MouseListener{
	...
	private int wait1=0;
	private int wait2=0;;
	private int fireWait1 = 0;
	private int fireWait2 = 0;
	...
	public GraphicsPanel(){
		...
		katze = new Sprite("sprite/katze/", -100,368,100,1,1);
	...
	public void clock(){
		...
		if(p2Block) {
			...
			if(blockCount2==300) {
				...
				if(p2Block && katze.collision(p1Fire)) {
					katze.shield();
					p1Fire = null;
	...
	public void keyPressed(KeyEvent e) {
		...
		if(!skinWalker.isDead&&!katze.isDead) {
			if (State == STATE.GAME) {
				...
				else if(e.getKeyCode()==KeyEvent.VK_SLASH&&fireWait1 <= 0) {
					if(skinWalker.x_direction<0) {
						p1Fire = new Item(skinWalker.x_coordinate-150, skinWalker.y_coordinate, "images/objects/fireball!.png", 1);} // images
					else {
				...
				else if(e.getKeyCode()== KeyEvent.VK_F&&fireWait2 <= 0) {//fire 
					if(katze.x_direction<0) {
						p2Fire = new Item(katze.x_coordinate-150, katze.y_coordinate, "images/objects/fireball!.png", 1);} // images
					else {
