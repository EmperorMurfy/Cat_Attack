...
// Description: Mainly intended for background music purposes, plays on loop with volume control
...
public class playMusic {
  ...
  public void run() {
    try {
      ...
      // Set initial volume (adjust between -80.0f and 6.0f
      setVolume(0.0f); // 0.0f represents no change
  ...
  public void setVolume(float volume) {
    if (clip == null) {
      throw new IllegalStateException("Audio has not been runned yet!");
    }
    ...
    volume = Math.max(min, Math.min(volume, max));
